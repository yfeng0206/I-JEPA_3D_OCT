...
**Statistical analysis**: [ablation_analysis.md](ablation_analysis.md) has bootstrap 95% CIs and paired-delta p-values for every comparison. CrossAttnPool beats d=1 significantly (p=0.002); MeanPool vs d=1 is NS (p=0.08) — we can't claim d=1 is worse, only that it fails to improve over MeanPool at 3000× more params.
...
3. **Frozen-probe ceiling is ~0.87-0.88 Test AUC on this dataset.** Fine-tune adds the next 1-2%.
