...
| [MIRAGE anatomy ep30](anatomy_ep30.md) | `mirage_anatomy`: connected anatomy-shaped blobs | Warm-start envelope `resume-ep27.pth.tar` | 30 so far | Stopped at ep30 | **Yes** — `D:\jepa_phase0\runs\patch_mirage_anatomy\` has `ep30.pth.tar` and `best.pth.tar` only | Matched ep30: 0.8582 ± 0.0003 |
...
The ep30 anatomy-vs-envelope comparison is the only fair head-to-head currently available. The ep100 historic numbers compare different training lengths for the anatomy question because the anatomy arm is stopped at ep30.
...
4. **No early stopping for fixed comparisons.** The 100-epoch arms use fixed epochs and saved checkpoints; the current anatomy arm is stopped at ep30 and should only be compared to matched ep30 baselines.
