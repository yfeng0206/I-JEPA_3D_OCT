...
### 22. A bug fix or speed optimisation must apply to every arm in the comparison, or run as a separate experiment
- **What happens:** `amp_target` (fp16 EMA-teacher forward, 1.66× speedup) and `enc_truncate: window` (a crop-offset fix) were both enabled only for the COVER-then-RANDOM campaign, while every archived baseline (random, oracle, envelope, blob ep1→56) kept the old behaviour. The COVER config even had a comment stating the rule directly above the setting — *"a single arm must not differ in how its targets are computed. Turn on only when every arm in the comparison uses it"* — and was violated on the next line. The campaign's ep30/50/75/100 AUCs (0.8558/0.8590/0.8612/0.8607, last of all arms) are consequently unattributable to the masking method: see [`docs/experiments/masking/crop_and_precision_audit.md`](experiments/masking/crop_and_precision_audit.md).
- **Rule:** Never apply a bug fix or speed optimisation to only the arm under test — apply it to every arm in the comparison, or run it as its own, separately-attributed experiment. Writing the confound into a deviation ledger documents it; it does not substitute for avoiding it.

### 23. Validate an intervention on the metric you care about, not a proxy
- **What happens:** `enc_truncate: window` was adopted because it improved a mask statistic (zero-anatomy rate) — but a full six-arm mask-statistics pass (`scripts/arm_stats_table.py`, 1,534 slices) later showed COVER's `prefix` zero-anatomy rate (11.02%) was already statistically alongside envelope's (10.10%), and envelope — the worst-blanking rectangle arm — still scored the second-best downstream AUC. The mask-statistic the fix targeted was never shown to predict AUC before the intervention was adopted.
- **Rule:** Same class as #20 — validate a masking or sampling intervention on downstream AUC, not only on a proxy such as a mask statistic. If a proxy is used to screen candidates cheaply, treat any resulting change as unconfirmed until checked against AUC.
