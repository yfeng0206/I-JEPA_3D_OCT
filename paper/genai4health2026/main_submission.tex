...
block is fp32 and its $\Delta$ therefore crosses the precision boundary; those
rows are marked $\ddagger$ and are excluded from headline claims.}
\label{tab:main}
\footnotesize
...
\midrule
\textsc{anatomy-v2} & segmenter (target shape) & \AUCAnatomyTwoEpFifty & $\DAnatomyTwoRandomEpFifty^{\ddagger}$ & --- & --- & --- & --- \\
\textsc{cover} $f{=}.21$ & segmenter (coverage) & \AUCCoverEpFifty & $\DCoverRandomEpFifty^{\ddagger}$ & --- & --- & \ph{pending} & \ph{pending} \\
\bottomrule
\end{tabular}
\end{table}
...
We are careful not to overstate this. \textsc{anatomy-v2} is not \emph{worse}
than the null at the matched epoch; it is statistically indistinguishable from
it (CI $\DAnatomyTwoRandomEpFiftyCI$, $p=\DAnatomyTwoRandomEpFiftyP$), as is
\textsc{cover} (CI $\DCoverRandomEpFiftyCI$, $p=\DCoverRandomEpFiftyP$). Both of
those intervals cross the fp16/fp32 boundary and so are quoted with the
$\ddagger$ caveat of Table~\ref{tab:main}; Section~\ref{sec:precision} shows the
precision effect is roughly two orders of magnitude smaller than the interval
width, so the conclusion does not turn on it. What the data rule out is not that
these policies are harmful, but the expected \emph{monotone improvement with
anatomical precision}: the intervals are centred near zero and comfortably
contain it, while the two cruder policies produce intervals that exclude it.
...
epochs. The two fp32 arms are marked $\ddagger$ as in Table~\ref{tab:main}.}
\label{tab:geom}
\small
...
\textsc{cover}    & 74.1\% & 45.3\% & 43.3\% & 43.5\% & 160.0 & $\AUCCoverEpFifty^{\ddagger}$ \\
\textsc{anatomy-v2} & 80.3\% & \textbf{97.3\%} & 21.4\% & 67.9\% & 64.0 & $\AUCAnatomyTwoEpFifty^{\ddagger}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Fine-tuning narrows but does not erase the gap}
...
anatomy coverage --- does not add to it. The practical reading, bounded by the
caveats below, is that for \emph{this} task the segmentation stage bought us
nothing we could measure, and a one-line intensity statistic recovered the whole
benefit at a fraction of the engineering cost. We would not generalise that to
tasks where anatomy is the output --- layer segmentation, lesion localisation,
progression --- where segmentation-aware pretraining may well matter.
...
mistake optimisation noise for method effects, and that the variance that matters
is the one induced by re-running training, not by resampling the test set. That
is precisely the variance we cannot measure here. This is the principal
limitation of the study, it applies to every arm comparison we report, and the
single most valuable follow-up is a seed replicate of the two policies that
...
indistinguishable from not guiding at all; and the best policy in the study
consulted no segmentation model. Direct measurement of mask geometry shows the
ordering is uncorrelated with how much anatomy is hidden, and a subgroup audit
over \NprobesSub{} probes shows the gains do not reach the groups or the disease
stage that need them most. We suggest the operative variable may be the
consistency of the predictive task rather than its anatomical correctness, and we
will release weights and the reproduction protocol so the claim can be checked.
