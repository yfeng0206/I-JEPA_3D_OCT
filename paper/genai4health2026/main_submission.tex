...
\NprobesSub{} probes finds the same worst-served group every time; all three
...
\item The subgroup audit is a caution: every policy leaves the same groups
...
$\SubAUCMin$ to $\SubAUCMax$, no masking policy reorders the subgroups.
...
\caption{\textbf{Left:} race-stratified AUC at epoch 100. Error bars are
3{,}000-resample class-stratified percentile bootstrap intervals within each
group, with fitted heads fixed and
per-group $n$ in the legend; the bars sit on a truncated axis, so bar
\emph{height} exaggerates the differences and only the plotted positions should
be read. The black subgroup is lowest under every policy, yet
...
over the \NprobesRace{} probes that carry a race summary. The dashed line is a
