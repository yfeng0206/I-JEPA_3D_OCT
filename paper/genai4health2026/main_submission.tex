...
blocks. A context encoder embeds the visible tokens; a predictor maps that
...
(Table~\ref{tab:geom}) --- a much easier prediction task. Any of these could
...
irregular target shape is harmful. Figure~\ref{fig:geom} makes the four-way
divergence explicit.
...
Table~\ref{tab:main} to within $0.0009$. The observed margin is larger with fewer
labels: at $5\%$ of the labelled set ($n{=}\LENFive$) \ArmBest{} leads the null by
...
(branch-level $\rho=\SubGenderBranchRho$, $q=\SubGenderQ$). Neither harm nor
benefit to equity is established here.
