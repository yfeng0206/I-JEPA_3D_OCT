...
100 (CI $\DEnvelopeRandomEpHundredCI$). \ArmBest{}, the segmentation-free
intensity band, gains $\DOracleRandomEpHundred$ at epoch 100
($\DOracleRandomEpHundredCI$). Aiming is not containment: the rectangles
are large, so only $43.3\%$ of \textsc{envelope}'s masked cells land on tissue
(Table~\ref{tab:geom}). Why the null stays this close is examined in
Appendix~\ref{app:bg}.
