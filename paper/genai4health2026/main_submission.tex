...
\title{Location, Not Shape: A Controlled Study of Anatomy-Guided\\
...
pre-specified contrasts against the null, at all three epochs, exclude zero. But
...
block is fp32 and its $\Delta$ is taken against an fp32 null re-probed for that
comparison, so those rows are matched on epoch and precision too.}
\label{tab:main}
\footnotesize
...
\textsc{cover} $f{=}.21$ & segmenter (coverage) & \AUCCoverEpFifty & \DCoverRandomEpFifty & \AUCCoverEpSeventyFive & $\mathbf{\DCoverRandomEpSeventyFive}$ & \ph{pending} & \ph{pending} \\
\bottomrule
\end{tabular}
\end{table}
...
defensible gap \emph{trend}: the race gap correlates with aggregate AUC at
$\rho=\SubRaceRho$ ($p=\SubRaceRhoP$) across checkpoints, but this fails
Benjamini--Hochberg correction over the seven attributes tested
($q=\SubRaceQ$) and vanishes when pseudo-replication is removed by averaging
within each of the \NbranchesSub{} branches ($\rho=\SubRaceBranchRho$,
$p=\SubRaceBranchP$). The only gaps that survive both are sex and severity, and
both \emph{narrow}. We do not claim that better masking harms any group, and we
do not claim it helps equity either.

\paragraph{Early disease improves, but stays hard.} Scoring each severity
stratum against the shared pool of all negatives, the strongest policy improves
mild disease from $\SevRandomMild$ to $\SevOracleMild$ ($\SevDeltaMild$ at
matched epoch 100), moderate by $\SevDeltaModerate$ and severe by
$\SevDeltaSevere$: mild disease improves \emph{most}. What does not move is the
difficulty ordering. The severe-to-mild gap is $\SevGapRandom$ for the null and
$\SevGapOracle$ for the best policy, and stays within $\SeverityGapSpread$ of
constant across all \NprobesSub{} probes while aggregate AUC spans
$\SubAUCMin$--$\SubAUCMax$. Target placement lifts the whole curve; it does not
change its shape. Appendix~\ref{app:subgroup} gives all seven stratifications.
These are secondary, descriptive analyses; no policy here was designed as a
...
hidden, and a subgroup audit over \NprobesSub{} probes shows the gains do not
reach the groups or the disease stage that need them most. We suggest the
...
\begin{table}[h]
\centering
\caption{All frozen probes. Protocol identical throughout: FairVision test
$N{=}3000$, seed 42, frozen encoder, MeanPool + LayerNorm + Linear.}
\label{tab:allprobes}
\small
\begin{tabular}{llcc}
\toprule
...
\midrule
ancestor (\textsc{random} @ep25) & 25 & 0.848680 & fp32 \\
\textsc{random}   & 50 & 0.864097 & fp32 \\
\textsc{random}   & 75 & 0.872302 & fp32 \\
\textsc{random}   & 100 & 0.874581 & fp32 \\
\textsc{intensity}   & 50 & 0.874030 & fp32 \\
\textsc{intensity}   & 75 & 0.883636 & fp32 \\
\textsc{intensity}   & 100 & \textbf{0.885485} & fp32 \\
\textsc{envelope} & 30 & 0.853917 & fp32 \\
\textsc{envelope} & 50 & 0.876064 & fp32 \\
\textsc{envelope} & 75 & 0.880307 & fp32 \\
\textsc{envelope} & 100 & 0.880743 & fp32 \\
\textsc{anatomy-v1} & 30 & 0.858274 & fp32 \\
\textsc{anatomy-v2} & 35 & 0.866129 & fp32 \\
\textsc{anatomy-v2} & 40 & 0.868251 & fp32 \\
\textsc{anatomy-v2} & 50 & 0.865386 & fp32 \\
\textsc{anatomy-v2} & 75 & 0.862492 & fp16 (confounded) \\
\textsc{anatomy-v2} & 92 & 0.860364 & fp16 (confounded) \\
\textsc{cover} $f{=}.21$ & 27 & 0.848347 & fp32 \\
\textsc{cover} $f{=}.21$ & 30 & 0.852249 & fp32 \\
\textsc{cover} $f{=}.21$ & 34 & 0.857083 & fp32 \\
\textsc{cover} $f{=}.21$ & 50 & 0.864281 & fp32 \\
\bottomrule
\end{tabular}
\end{table}
...
of one another, which is the direct signature of laterality flipping rather than
bilateral anatomy.}
...
and the best-performing policy does not close any gap. We would regard deployment
...
\ph{fp32 re-probe table pending}
