...
arms exclude zero at all three epochs. Anatomical \emph{precision} does not add. A policy that places
...
the unadjusted white race-group interval excludes zero. The gains are not
...
\item Masking policy is isolated in medical SSL by a controlled six-arm study
      with a shared ancestor checkpoint and one evaluation protocol (every probe
...
\item Guidance helps in these runs; anatomical precision does not reliably add
      to it. The gain is carried by the control that separates ``avoid
      background'' from ``target anatomy'': rectangles aimed at retinal tissue.
\item The most anatomically precise policy is not the best performer, from
      direct measurement of mask geometry for every policy on 600 slices, with
...
of increasing anatomical specificity, identical in every other respect.
...
Region matters and coverage does not. Each policy was pretrained once, so the
...
per-arm error bars: marginal intervals carry between-case variance
...
\textbf{Anatomy tells you where to aim, not how much to cover.} Of the four
guided arms the best performer hides the \emph{least} anatomy, while the arm
...
subsets fixed --- the subsampling seed depends on the fraction and repeat but
never on the arm --- and, across all four arms in this sweep, the
...
most. Unadjusted paired intervals exclude zero only for the white subgroup
...
and their unadjusted intervals exclude zero (Table~\ref{tab:pairedsub}). Whether mild
...
In these runs anatomy mattered for \emph{where} targets are aimed and not for
\emph{how much} anatomy they cover. Under a matched schedule, effective batch
...
model, while shaping targets to trace the segmented retina did not separate from unguided
...
positive disease-stage point estimates with unadjusted intervals excluding zero,
...
rise and their unadjusted paired intervals exclude zero
(Table~\ref{tab:pairedsub}), the largest point estimate at mild
...
\ArmBest{} at matched epoch 100. The eight rows plus two gain-difference CIs in
the text form one unadjusted exploratory family; no simultaneous coverage is
claimed. Each percentile CI uses \Nboot{} class-stratified subject resamples,
shared across both fixed heads and every stratum.}
\label{tab:pairedsub}
\small
...
validation-selected threshold, by stratum. These five unadjusted exploratory
CIs use \Nboot{} percentile paired resamples of positive subjects, with fitted
heads and threshold fixed. Epoch 100.}
\label{tab:subop}
\small
