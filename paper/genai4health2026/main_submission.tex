...
\NprobesSub{} probes finds the same worst-served group in every probe for five of
seven attributes; all three
...
\item The most anatomically precise policy is not the best performer, from
      ...
      \item The subgroup audit is a caution: for five of seven attributes every policy
      leaves the same group worst-served. Public-weight reproduction matches the headline result to
...
\begin{figure}[t]
\centering
\includegraphics[width=0.47\linewidth]{fig1_policies_compact.png}
\caption{The six masking policies on one identical B-scan, drawn with the
production samplers; each target block has its own colour and the green
contour is the anatomy reference. Anatomical specificity increases along the
ladder, while downstream AUC does not (Table~\ref{tab:main});
...
real capacity on them: in the one arm where we measured it, background
self-similarity falls from $0.784$ untrained to $0.346$ at epoch 100 under
\textsc{envelope}.
...
$\SubAUCMin$ to $\SubAUCMax$, the worst-served group is the same in every probe
for five of the seven attributes; language and age each dissent in a handful
...
reorganising background: self-similarity falls from $0.784$ untrained to $0.346$
at epoch 100. That second measurement was made on the \textsc{envelope} encoder,
the only arm for which the class-relation analysis was run, so it evidences that
pretraining reorganises background rather than that the null arm specifically
does.
...
contribution. For five of the seven attributes audited, every masking policy we
tested leaves the same group worst-served,
