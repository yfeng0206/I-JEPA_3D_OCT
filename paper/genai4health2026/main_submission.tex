...
\subsection{Guidance helps; anatomical precision does not}
...
\textsc{envelope}'s $77.6\%$ --- and the arm never realised the coverage it was
...
Table~\ref{tab:main} to within $0.0002$.

The margin grows sharply as labels are removed. At $5\%$ of the labelled set
...
ordering and not its size. The absolute gains at full supervision are small, but
the regime that matters for a new imaging site or a rare condition is the one
with few labels, and that is where choosing \emph{where} to predict pays.
