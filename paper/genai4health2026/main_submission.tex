...
\textbf{H2 fails.} H2 predicts the anatomy arms exceed \textsc{envelope}; they do
not, transitively through the shared null. At epoch 50 --- the only epoch at
which all five arms have a probe --- \textsc{envelope} beats \textsc{random} by
$\DEnvelopeRandomEpFifty$ with an interval excluding zero, while
\textsc{anatomy-v2} gains only $\DAnatomyTwoRandomEpFifty$ and \textsc{cover}
$\DCoverRandomEpFifty$, both indistinguishable from that null. An arm that does
not separate from the null cannot exceed one that beats it, so H2 is refuted
without a separate anatomy-versus-envelope contrast. Against
$\DOracleRandomEpFifty$ for \ArmBest{}, policies that merely \emph{place}
rectangles on tissue gain roughly an order of magnitude more than those shaping
targets to match it.
...
null; both are indistinguishable from it, against an fp32 null re-probed for
this comparison, so they are matched on epoch \emph{and} precision. What the
data rule out is not harm but the expected \emph{monotone improvement with
anatomical precision}.

\paragraph{Carried to epoch 100 the coverage arm degrades, but a defect blocks
the natural reading.}
...
max--min gap is easy to misread as harm. It is not. At epoch 100 the strongest
...
the segmented retina left performance indistinguishable from not guiding at all,
and the best policy consulted no segmentation model. Mask geometry is
uncorrelated with how much anatomy is hidden; a subgroup audit shows every
...
resampling, the largest point estimate at mild disease ($\PDSevMildDelta$, CI
...
Guidance that only matters when labels are plentiful is of limited clinical
interest, so we measured how each policy behaves as supervision is withdrawn. We
...
against $\LEGapHundred$ at full supervision --- roughly a fourfold larger margin.
\textsc{envelope} behaves similarly ($\LEEnvelopeFive$ against
$\LERandomFive$). By $10\%$ the ordering has already closed to within noise and
the arms are indistinguishable.

Two cautions. At $1\%$ the standard deviations are large
($\LESDRandomOne$--$\LESDCoverOne$ across arms), so the ordering there is not
separated and we do not read it. And the defective \textsc{cover} arm
(Appendix~\ref{app:covbug}) is the worst arm at every fraction below $25\%$,
reaching only $\LECoverOne$ at $1\%$ against $\LERandomOne$ for the null, which
is consistent with its targets being degraded rather than merely differently
placed.
