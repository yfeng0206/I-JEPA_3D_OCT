...
\textbf{H2 fails.} H2 predicts the anatomy arms exceed \textsc{envelope}. Tested
directly at epoch 50 --- the only epoch at which all five arms have a probe ---
\textsc{anatomy-v2} sits $\DAnatomyTwoEnvelopeEpFifty$ \emph{below}
\textsc{envelope} (CI $\DAnatomyTwoEnvelopeEpFiftyCI$,
$p=\DAnatomyTwoEnvelopeEpFiftyP$), refuting the prediction in the opposite
direction rather than merely failing to support it. Against the shared null,
\textsc{envelope} gains $\DEnvelopeRandomEpFifty$ while \textsc{anatomy-v2} gains
$\DAnatomyTwoRandomEpFifty$ and \textsc{cover} $\DCoverRandomEpFifty$, neither
separable from it; \ArmBest{} gains $\DOracleRandomEpFifty$. Policies that merely
\emph{place} rectangles on tissue gain roughly an order of magnitude more than
those shaping targets to match it.
