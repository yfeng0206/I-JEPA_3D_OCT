...
and~\ref{fig:ladder}; provenance in Appendix~\ref{app:geomprov}). Two
observations follow.
...
supervision --- more than four times the advantage.
Whether the gap genuinely widens as labels are
...
deployment: the validation-selected threshold aimed at specificity $0.90$
realises $0.895$ for white and $0.736$ for black patients under the null, and
$0.879$ and $0.761$ under \ArmBest{}, so the smaller stratum absorbs a higher
false-positive rate under \emph{both} arms (Appendix~\ref{app:operating}).
...
--- but anatomical precision did not reliably. Restricting targets to retinal tissue
...
\paragraph{Cell-level agreement and seed noise.} Comparing the 25 geometry cells
of Table~\ref{tab:geom} against the regeneration, one cell reproduces exactly,
21 preserve the printed ordering of the arms on their metric, and 3 do not;
18 of the 25 fall inside the three-seed range of the measurement, i.e.\ are
indistinguishable from a re-draw. The largest absolute difference over all 25
cells is $2.21$ points. All three cells whose ordering moves are the
\emph{loss slots} column on rectangle arms (Table~\ref{tab:geomprov}), whose
printed spread across those arms is smaller than the per-arm seed spread. That
ordering therefore carries no information, and no claim in this paper depends on
it.

\begin{table}[h]
\centering
\caption{Regeneration of two Table~\ref{tab:geom} columns against three
independent draws of the same 600-slice measurement. ``sd'' is the standard
deviation over seeds 42, 1234 and 2026. The loss-slot differences between
rectangle arms are smaller than this noise.}
\label{tab:geomprov}
\small
\begin{tabular}{lcccccc}
\toprule
 & \multicolumn{3}{c}{anatomy hidden (\%)} & \multicolumn{3}{c}{loss slots} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}
policy & printed & regenerated & sd & printed & regenerated & sd \\
\midrule
\textsc{random}     & 52.2 & 53.95 & 0.71 & 157.7 & 159.91 & 1.15 \\
\ArmBest{}          & 62.2 & 62.13 & 0.64 & 158.4 & 158.99 & 0.43 \\
\textsc{envelope}   & 76.9 & 77.58 & 0.38 & 159.9 & 159.68 & 1.00 \\
\textsc{cover}      & 74.1 & 73.55 & 0.30 & 160.0 & 159.09 & 0.45 \\
\textsc{anatomy-v2} & 80.3 & 79.89 & 0.39 & 64.0  & 64.00  & 0.00 \\
\bottomrule
\end{tabular}
\end{table}
...
than a neutral shrink. \emph{Provenance:} the pre- and post-truncation coverage
figures and the retained-target statistics in this paragraph come from a one-off
CPU audit of 194 accepted slices whose raw output was not persisted, so they
cannot be recomputed from the released artifacts. The $73.1\%$ against $77.6\%$
...
specificity and calibration of Appendix~\ref{app:operating}, it contains no
