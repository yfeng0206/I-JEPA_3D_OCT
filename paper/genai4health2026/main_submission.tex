...
fork from the same epoch-25 checkpoint (open circle). Rectangle arms differ
only in masking policy; the anatomy arms also differ in collation. Solid lines are the precision-matched fp16 family; dashed lines are fp32
...
A $+0.011$ AUC difference is hard to interpret clinically, whereas a change in
cases detected at a fixed threshold is not. We state that comparison carefully:
