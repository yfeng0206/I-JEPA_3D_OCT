...
(Table~\ref{tab:geom}). Avoiding background is worth something at every epoch we
measured, with an interval that excludes zero.
...
the segmented retina matched unguided masking at epoch 50 and fell below it by
epoch 75, and the best policy consulted no segmentation model. Mask geometry is
...
\paragraph{These AUCs are not comparable to Table~\ref{tab:main}.} This sweep
fits a simpler head than the primary protocol --- full-batch, fixed learning
rate, no validation-based epoch selection --- because it must be repeated
\LERepeats{} times at each of five fractions. That changes the absolute numbers:
at full supervision it puts \textsc{random} at $\LERandomHundred$ against
$\AUCRandomEpHundred$ in the primary protocol, which compresses the
\ArmBest{}-over-null gap from $\DOracleRandomEpHundred$ to about $+0.004$. Only
the \emph{within-protocol} comparison across fractions should be read from this
table. We report the discrepancy rather than suppressing it, because it shows the
headline margin is sensitive to how the probe is fitted --- a caveat that applies
to any frozen-probe comparison at this effect size, including ours.
