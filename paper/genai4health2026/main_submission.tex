...
so over-constraining where the model may look is worse than not guiding at all. Direct measurement of mask geometry across
...
\paragraph{Carried to epoch 100, coverage-constrained masking degrades.}
\textsc{cover} is the only segmenter-driven policy we continued to the full
...
(Figure~\ref{fig:traj}). The gap against the null therefore widens with training:
...
$p\,\DCoverRandomEpHundredP$), where it also sits $\DCoverOracleEpHundred$ below
\textsc{intensity}. All contrasts are matched on epoch and precision.

Forcing a fixed fraction of tissue to stay visible does not merely fail to help.
Once the easy signal is exhausted the constraint appears to cap what the encoder
can still learn, and continued training then erodes the representation. This is
the sharpest form of our H2 result: more aggressive use of the segmentation model
is not neutral here but actively harmful. Caveats: one floor value ($f{=}0.21$),
so this concerns that floor rather than a dose-response; and a single
continuation, as everywhere.
...
The long-horizon and anatomy arms were probed at different autocast settings
(Section~\ref{sec:setup}), so we measured the precision effect rather than
assuming it away: a full re-fit of the probe pipeline at fp32 shifts every arm by
less than $2\times10^{-4}$, two to three orders of magnitude below the effects in
Table~\ref{tab:main}, and every contrast we report is now matched on precision.
Appendix~\ref{app:fp32} gives the numbers.
