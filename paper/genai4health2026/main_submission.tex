...
% segmentation model and NO ground-truth labels. The name is historical; see the
% definition at first use in Section 3.
\newcommand{\ArmBest}{\textsc{oracle}}
...
\item[\ArmBest{} (segmentation-free).] A band whose vertical position is
  ...
  It uses no ground-truth annotation of any kind. (The name is historical, from
  when this policy was intended as an upper bound, and we keep it so that the
  paper matches the released code and checkpoints. It should not be read as
  implying label access: despite the name, this arm is \emph{not} an oracle and
  \emph{not} an upper bound, and reads only the input image.)
...
\ArmBest{}. The policies that merely \emph{place} rectangles on tissue gain
...
\paragraph{Carried to epoch 100, the coverage-constrained policy degrades ---
but an implementation defect blocks the natural reading.}
...
\ArmBest{}. All contrasts are matched on epoch and precision.

The obvious reading --- that pushing coverage higher is actively harmful --- is
\emph{not} available to us. An audit (Appendix~\ref{app:covbug}) found the
collator shortens every predictor target \emph{after} \textsc{cover} chooses its
placement, defeating its greedy optimisation: delivered masks hide $73.1\%$ of
anatomy, \emph{less} than \textsc{envelope}'s $77.6\%$, not the $78.6\%$ the
placement achieved. \textsc{cover} never realised the aggressive coverage it was
built to test, so its decline cannot be attributed to over-covering anatomy, and
because coverage was logged before truncation every training log reported the
intent. What survives is narrower: a coverage-seeking policy whose targets are
shortened and top-biased peaks early and then erodes while the null improves.
...
from the null; \ArmBest{} places $41.1\%$ on tissue and gives the largest
gain in the study.
...
Crucially, these are not consequences of blob geometry: the anatomy arms resample
...
The epoch-100 \ArmBest{} encoder and its trained probe head will be
...
mechanism behind \ArmBest{}'s advantage. Two explanations remain live:
...
\paragraph{Measured consequences.} On held-out slices, \textsc{cover}'s placement
...
untrained to $0.346$ at epoch 100, while tissue contrast collapses.
...
every other. A background-only probe scores $0.867$, but that is attention
leakage rather than background signal.
