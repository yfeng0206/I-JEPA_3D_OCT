...
def shrink_to_k(mask, k, score=None):
    """Keep at most `k` cells of `mask`, staying CONNECTED.

    Returns exactly `k` cells whenever `mask` is a single connected component,
    which is what `build_targets` always produces via `geodesic_partition`.
    A disconnected input yields only the cells reachable from the seed, so the
    result can be smaller than `k`; collation's `resample_to_k` then tops it up
    with replacement.

    Fixed-K collation subsamples targets larger than k.  Doing that uniformly
    destroys the property the method exists for: a connected 24-cell target
    sampled down to 16 random cells is no longer connected, and measurement
    showed only 231/256 targets surviving as single components.  Growing a
    connected subset breadth-first from the highest scoring cell keeps the
    anatomical shape, and restores 256/256.
    """
    ...
    seed = tuple(coords[int(np.argmax([sc[tuple(c)] for c in coords]))])
    h, w = m.shape
    out = np.zeros_like(m)
    out[seed] = True
    frontier = deque([seed])
    n = 1
    while frontier and n < k:
        r, c = frontier.popleft()
        nbrs = [(r + dr, c + dc)
                for dr in (-1, 0, 1) for dc in (-1, 0, 1)
                if not (dr == 0 and dc == 0)]
        nbrs = [(rr, cc) for rr, cc in nbrs
                if 0 <= rr < h and 0 <= cc < w and m[rr, cc] and not out[rr, cc]]
        nbrs.sort(key=lambda p: -sc[p])
        for p in nbrs:
            if n >= k:
                break
            out[p] = True
